...
This project uses the `agent-pipeline-claude` plugin's `module-release` pipeline (or `feature` / `bugfix`) for execution discipline. The audit-handoff protocol layered on top:
